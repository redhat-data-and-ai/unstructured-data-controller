...
func (r *ChunksGeneratorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Add force reconcile to VectorEmbeddingsGenerationJob CR
	vectorEmbeddingsJobKey := client.ObjectKey{Namespace: chunksGeneratorCR.Namespace, Name: chunksGeneratorCR.Spec.DataProduct}
	if err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		vectorEmbeddingsJobCR := &operatorv1alpha1.VectorEmbeddingsGenerationJob{}
		if err := r.Get(ctx, vectorEmbeddingsJobKey, vectorEmbeddingsJobCR); err != nil {
			return err
		}
		return controllerutils.AddForceReconcileLabel(ctx, r.Client, vectorEmbeddingsJobCR)
	}); err != nil {
		logger.Error(err, "failed to add force reconcile label to VectorEmbeddingsGenerationJob CR")
		return r.handleError(ctx, chunksGeneratorCR, err)
	}
