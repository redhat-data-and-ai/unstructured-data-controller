...
const (
	configControllerName = "controllerconfig"
)
...
func (r *ControllerConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	r.mustReconcileControllerGate(config.Spec.DisabledControllers)

	if isControllerDisabled(configControllerName) {
		logger.Info("config controller is disabled, skipping reconciliation")
		return ctrl.Result{}, nil
	}
...
func (r *ControllerConfigReconciler) mustReconcileControllerGate(disabledControllerNames []string) {
	if r.ctrlGate == nil {
		r.ctrlGate = getSharedControllerGate()
	}
	r.ctrlGate.setDisabledControllers(disabledControllerNames...)
}
