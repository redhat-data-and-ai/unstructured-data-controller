...
vet: ## Run go vet against code.
	go vet -tags minicore_disabled ./...

.PHONY: test
test: manifests generate fmt vet setup-envtest ## Run tests.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" go test -tags minicore_disabled $$(go list ./... | grep -v /e2e) -coverprofile cover.out
...
build: manifests generate fmt vet ## Build manager binary.
	go build -tags minicore_disabled -o bin/manager cmd/main.go

.PHONY: run
run: manifests generate fmt vet ## Run a controller from your host.
	go run -tags minicore_disabled ./cmd/main.go
